...
function renderMascotHeader(config, className) {
  return `<header class="${className}-head"><div><p class="level-layout-kicker">${escapeHtml(config.kicker)}</p><h1 id="ui-lab-content-title">${escapeHtml(config.title)}</h1><p>${escapeHtml(config.prompt)}</p></div><div class="${className}-mascot"><div class="${className}-bubble">${escapeHtml(config.mascot)}</div><img src="assets/mascots/chibi-placeholder.png" alt="Friendly lesson mascot" /></div></header>`;
}
